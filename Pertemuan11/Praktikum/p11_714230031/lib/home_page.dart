...
class _HomePageState extends State<HomePage> {
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (res != null) {
                          ...
                          displaySnackbar('Gagal mengambil data user');
                        }
                      },
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        if (_nameCtl.text.isEmpty || _jobCtl.text.isEmpty) {
                          displaySnackbar('Nama dan Job tidak boleh kosong');
                          return;
                        }
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        UserCreate? res = await _dataService.putUser(
                          '3',
                          _nameCtl.text,
                          _jobCtl.text,
                        );
  ...
  Widget hasilCard(BuildContext context) {
    return Column(
      children: [
        if (_usCreate != null)
          UserCard(usrCreate: _usCreate!, isUpdate: false)
        else if (_usrUpdate != null)
          UserCard(usrCreate: _usrUpdate!, isUpdate: true)
        else
          const Text('no data'),
      ],
    );
